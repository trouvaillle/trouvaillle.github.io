(function() {
    ...
    let sendElement;
    let chatElement;
    let sendByDataConnection;

    function appendChat(chatElement, value) {
        let pElement = document.createElement('p');
        pElement.innerText = value;
        chatElement.appendChild(pElement);
    }

    function clearChat(chatElement) {
        chatElement.children.remove();
    }

    function sendWords(value) {
        if (value.length == 0) {
            return;
        }
        sendToWebRTC(value);
        // appendChat(chatElement, value);
        scrollToEnd(chatElement.parentElement);
        clearWords(wordsElement);
    }

    function clearWords(wordsElement) {
        wordsElement.value = '';
    }
    ...
    window.onload = () => {
        ...
        wordsElement.addEventListener('keydown', (event) => {
            if (event.key == 'Enter') {
                sendWords(wordsElement.value);
            }
            
        });
        sendElement.addEventListener('click', (_) => {
            sendWords(wordsElement.value);
        });
    }

    function sendToWebRTC(value) {
        sendByDataConnection(value);
    }
    ...
    window.DataConnection = function(channel) {
        ...
        this.channel = channel || "trouvaillle-app-webrtc-chat-default-channel";
